...
const table = cva("w-full border-collapse relative dark:text-tertiary", {
  variants: {
    ...
    color: {
      primary:
        "hover:[&_tbody_tr]:bg-gray-50 dark:hover:[&_tbody_tr]:bg-primary/10",
      secondary: "hover:[&_tbody_tr]:bg-gray-100/70",
  ...
  compoundVariants: [
    {
      ...
      color: "primary",
      className:
        "odd:[&_tbody_tr]:bg-gray-50/50 dark:odd:[&_tbody_tr]:bg-primary/5",
    },
    {
      variant: "striped",
      color: "secondary",
      className:
        "odd:[&_tbody_tr]:bg-gray-50 dark:odd:[&_tbody_tr]:bg-dark-200/50",
    },
    {
      variant: "striped",
      color: "light",
...
const Head = ({ children, className, ...rest }: HeadProps) => {
  return (
    <thead
      className={cn(
        "w-full border-b border-gray-100/60 dark:border-dark-200",
        className
      )}
      {...rest}
    >
...
const Th = ({ children, className, ...rest }: ThProps) => {
  return (
    <th
      className={cn(
        "z-10 min-w-[25px] py-3 sticky top-0 text-[0.88rem] font-semibold capitalize text-gray-300 dark:text-white",
        className
      )}
      {...rest}
    >
